...
The default workflow in `SKILL.md` assumes a profitable single-name large-cap with at least five quarters of reported history. When that assumption breaks, keep the signal-monitor structure stable and swap the metric choices or framing details rather than falling back to vague prose. This file lists the four cases that come up most often and what to adjust.

## ETFs and index-tracking funds

The monitor badge vocabulary still applies, but P/E (TTM) and Revenue YoY are not meaningful for a fund. If the surrounding product contract expects action rows, keep them secondary.
...
The **Verified Facts** section sources from the fund prospectus (primary) and at least one independent commentary (Morningstar bylined article, Reuters fund coverage). The change-view section still applies, but the invalidation is usually a methodology change in the underlying index, not a single fundamental print.

## Loss-makers and pre-profitability names

When TTM diluted EPS ≤ 0, a P/E multiple is undefined and the formula column becomes a divide-by-near-zero booby trap. The metrics script already swaps in a P/Sales row automatically; mirror that in the monitor:
...
- Add a `Path-to-profitability` row in **Verified Facts** citing management's stated breakeven timing (IR letter or earnings transcript) so the monitor does not implicitly assume profitability is around the corner.
...
The metrics script returns `n/a` for Revenue YoY when fewer than five quarters are available rather than fabricating a comparable period. In the monitor:
...
- The change-view section should include an `Upgrade / Review Now` or holder-action line keyed to the first post-lockup print, since that is the most common next decision point.
...
Pro-forma history matters more than reported history. In the monitor:
